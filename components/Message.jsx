...
const Message = () => {
  return (
    <div className='flex flex-col items-center w-full max-w-3xl text-sm'>
        <div className={`flex flex-col w-full mb-8 ${roll==='user'&& 'items-end'}`}>
            <div className={`group relative flex max-w-2xl p-3 rounded-xl ${roll==='user'?'bg[#414156] px-5':'gap-3'}`}>
                <div className={`opacity-0 group-hover:opacity-100 absolute ${roll==='user'? '-left-16 top-2.5':'left-9 -bottom-6'}transition-all`}>
                    <div className='items-center flex gap-2 opacity-70'>
                        {
                            role==='user'?(
                                <>
